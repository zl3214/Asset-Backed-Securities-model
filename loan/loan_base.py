'''
Create a Loan base class
'''
from .asset.assets import Asset
import logging
...
class Loan(object):
    ...
    def balance_formula(self, period):
        if period > self._term:
            ...
            return 0 if self._defaulted else self._face * (1 + self._rate) ** period - self.monthlyPayment(period) * (((1 + self._rate) ** period - 1) / self._rate)
